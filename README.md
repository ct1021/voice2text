...
SenseVoice 中文识别更快更准，但模型需手动下载：

1. 搜索并下载 sherpa-onnx 的 SenseVoice int8 模型包
2. 解压后把 `model.int8.onnx` 和 `tokens.txt` 放进 `models/sensevoice/`
3. `config.toml` 设 `[stt] backend = "sensevoice"`，重启
